...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name guardianes-service apps/guardianes-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app guardianes-service:1.0-SNAPSHOT
    oc expose svc/guardianes-service
else
